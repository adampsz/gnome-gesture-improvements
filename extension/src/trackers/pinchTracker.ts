...
export const TouchpadPinchGesture = GObject.registerClass(
	...
	class TouchpadPinchGesture extends GObject.Object {
		...
		_handleEvent(
			...
			event: Clutter.Event | DBusUtils.SyntheticEvent,
		): boolean {
			if (event.type() !== Clutter.EventType.TOUCHPAD_PINCH) return Clutter.EVENT_PROPAGATE;
			...
			if (this._state === TouchpadState.IGNORED || !this.enabled) return Clutter.EVENT_PROPAGATE;
		...
		private _emitEnd(time: number) {
			...
			if ((endProgress - this._progress_scale) * velocity <= 0) velocity = ANIMATION_BASE_VELOCITY;
		...
		private _findPointForProjection(pos: number, velocity: number) {
			...
			if ((velocity > 0 ? prev : next) === initial) return velocity > 0 ? next : prev;
